...
# 用法示例：
#   # 交互式选择ES服务器
#   ./rebuild_amap_poi_index.sh
#
#   # 指定ES服务器（跳过选择菜单）
#   ./rebuild_amap_poi_index.sh 1  # 使用本地服务器
#   ./rebuild_amap_poi_index.sh 2  # 使用远程服务器1
#
#   # 自定义配置（环境变量优先级最高）
...
# 索引读别名
READ_ALIAS=${READ_ALIAS:-"amap_poi_read"}

# 索引写别名
WRITE_ALIAS=${WRITE_ALIAS:-"amap_poi_write"}
...
NEW_INDEX=${NEW_INDEX:-"amap_poi_${TIMESTAMP}"}

# 定位映射文件（相对脚本目录）
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
MAPPING_FILE="$SCRIPT_DIR/rebuild_amap_poi_index.json"
...
if [ "$DELETE_OLD" = "true" ]; then
  ...
  # 列出所有 amap_poi_* 索引
  OLD_INDICES=$(curl -sS $AUTH_PARAM "$ES_URL/_cat/indices/amap_poi_*?h=index" 2>/dev/null | awk 'NF>0 {print $1}' || true)
...
# 显示所有 amap_poi 相关索引
echo ""
echo "  当前所有 amap_poi 相关索引:"
curl -sS $AUTH_PARAM "$ES_URL/_cat/indices/amap_poi_*?v&s=index" | sed 's/.*/    &/'
